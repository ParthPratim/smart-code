...
SUBMIXTURES = ["flan2021", "t0", "niv2", "cot", "dialog"]
TEMPLATE_TYPES = ["zs_opt", "zs_noopt", "fs_opt", "fs_noopt"]
CONTEXT_LEN = 4096
TEMPERATURE = 1
HUB_TOKEN = None
HUB_USERNAME = None
...
def download_flan2022():
    print(
        "Check if FLAN 2022 dataset is already downloaded. If not, download it and load it"
    )
...
def compute_prompt_embeddings():
    ...
    print(
        "Check if prompts_embeddings directory is present and it contains embeddings for all submixtures. If not, compute embeddings for prompts."
    )
    ...
    if not already_computed:
        ...
        for submixture in SUBMIXTURES:
            ...
            submixture_data = load_dataset(
                f"kowndinya23/{submixture}-submix-{CONTEXT_LEN}"
            )["train"]
            ...
            end_time = time.time()
            print(
                f"Time taken to compute embeddings for {submixture}:",
                end_time - start_time,
            )
...
def get_task_embeddings(task_indices):
    ...
    try:
        ...
        embeddings = np.concatenate(
            [embeddings.reshape((1, -1)) for embeddings in embeddings], axis=0
        )
...
def get_task_totals(task_indices):
    ...
    for submixture in SUBMIXTURES:
        ...
        for task in submixture_task_indices.keys():
            ...
            for template_type in TEMPLATE_TYPES:
                if template_type in submixture_task_indices[task]:
                    task_totals[task] += len(
                        submixture_task_indices[task][template_type]
                    )
    return task_totals
...
def get_task_budgets(
    tasks, task_totals, embeddings, submod_fnc, num_tasks, num_instances
):
    ...
    if submod_fnc == "random":
        ...
        data_sijs = submodlib.helper.create_kernel(
            X=embeddings, metric="cosine", method="sklearn"
        )
        if submod_fnc == "fl":
            print("Instantiating facility location function...")
            submod_obj = submod_fn.facilityLocation.FacilityLocationFunction(
                n=embeddings.shape[0], mode="dense", sijs=data_sijs, separate_rep=False
            )
        elif submod_fnc == "gc":
            print("Instantiating graph cut function...")
            submod_obj = submod_fn.graphCut.GraphCutFunction(
            ...
            submod_obj = submod_fn.logDeterminant.LogDeterminantFunction(
                n=embeddings.shape[0], mode="dense", sijs=data_sijs, lambdaVal=1
            )

        print("Running the lazy greedy algorithm...")
        greedyList = submod_obj.maximize(
            budget=num_tasks, optimizer="LazyGreedy", show_progress=True
        )
    ...
    print(
        f"The total number of instances in the selected tasks is {all_tasks_total}. The total number of instances in the subset is {num_instances}."
    )
    if all_tasks_total < num_instances:
        raise Exception(
            "The total number of instances in the selected tasks is less than the number of instances in the subset. Something wrong."
        )

    print("Applying Taylor softmax on gains...")
    probs = taylor_softmax_v1(
        torch.from_numpy(np.array([gains]) / TEMPERATURE)
    ).numpy()[0]
            ...
            try:
                ...
                for template_type in TEMPLATE_TYPES:
                    if template_type in submixture_task_indices[task]:
                        template_counts.append(
                            len(submixture_task_indices[task][template_type])
                        )
                    else:
                        template_counts.append(0)
                if sum(template_counts) < task_budget:
                    raise Exception(
                        f"Task budget exceeds number of instances in the task({task})"
                    )
                ...
                if not is_fs_present:
                    ...
                    if template_counts[0] == 0:
                        ...
                        zs_opt, zs_noopt = budget_split(
                            task_budget, [template_counts[0], template_counts[1]]
                        )
                    ...
                    zs, fs = budget_split(
                        task_budget,
                        [
                            template_counts[0] + template_counts[1],
                            template_counts[2] + template_counts[3],
                        ],
                    )
                    if template_counts[0] == 0:
                        if template_counts[1] == 0:
                        ...
                        zs_opt, zs_noopt = budget_split(
                            zs, [template_counts[0], template_counts[1]]
                        )
                        template_budgets[0] = zs_opt
                        template_budgets[1] = zs_noopt
                    if template_counts[2] == 0:
                        ...
                        fs_opt, fs_noopt = budget_split(
                            fs, [template_counts[2], template_counts[3]]
                        )
...
def load_instances_submodular_ordering(submod_fnc_instances):
    ...
    try:
        print(
            f"Trying to load submodular ordering of instances from {submod_fnc_instances}_ordering directory..."
        )
        for submixture in SUBMIXTURES:
        ...
        raise Exception(
            f"Submodular ordering for instances not found for {submod_fnc_instances}"
        )
...
def get_subset_indices(submodular_ordering, task_template_budgets, task_indices):
    print(
        "Getting subset of instances from each task based on submodular ordering and task_template_budgets..."
    )
    indices = {}
    for submixture in SUBMIXTURES:
        indices[submixture] = []
        ...
        for task in submixture_task_template_budgets.keys():
            for i, template_type in enumerate(TEMPLATE_TYPES):
                if template_type in submixture_ordering[task]:
                    ...
                    if task_template_budget == len(
                        submixture_task_indices[task][template_type]
                    ):
                        indices[submixture].extend(
                            submixture_task_indices[task][template_type]
                        )
                    else:
                        indices[submixture].extend(
                            [
                                idx
                                for idx, _ in submixture_ordering[task][template_type][
                                    :task_template_budget
                                ]
                            ]
                        )
    return indices
...
def get_final_dataset(indices):
    ...
    for submixture in SUBMIXTURES:
        ...
        submixture_train_datasets.append(
            submixture_data["train"]
            .select(indices[submixture])
            .remove_columns(["task_source", "task_name", "template_type"])
        )
        submixture_val_datasets.append(
            submixture_data["validation"].remove_columns(
...
def main():
    ...
    # Get task budgets
    task_budgets = get_task_budgets(
        tasks,
        task_totals,
        embeddings,
        args.submod_fnc_tasks,
        args.num_tasks,
        args.num_instances,
    )
